...
public class ColorPane extends JTextPane {
	 ...
	 * Serial version UID.
	 */
	private static final long serialVersionUID = 3181197949314798614L;
	...
	public void appendANSI(String s) { // convert ANSI color codes first
		...
		if (addString.length() > 0) {
			...
			if (aIndex == -1) { // no escape/color change in this string, so
								// just send it with current color
				append(colorCurrent, addString);
				return;
			}
			// otherwise There is an escape character in the string, so we must
			// process it

			if (aIndex > 0) { // Escape is not first char, so send text up to
								// first escape
			...
			while (stillSearching) {
				mIndex = addString.indexOf("m", aPos); // find the end of the
														// escape sequence
				if (mIndex < 0) { // the buffer ends halfway through the ansi
									// string!
				...
				aPos = mIndex + 1;
				// now we have the color, send text that is in that color (up to
				// next escape)

				aIndex = addString.indexOf("\u001B", aPos);

				if (aIndex == -1) { // if that was the last sequence of the
									// input, send remaining text
					tmpString = addString.substring(aPos, addString.length());
					append(colorCurrent, tmpString);
					stillSearching = false;
					continue; // jump out of loop early, as the whole string has
								// been sent now
				}
